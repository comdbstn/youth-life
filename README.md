...
- 나중에 고치기로 하고 일단 넘어가기
- **날짜 필터링으로 데이터를 숨기기** (v1.2에서 발견 및 수정)
...
- 설정이 필요하면 명확한 가이드 제공
- **필터링은 사용자가 기대하는 대로 작동** (예: "미완료 태스크"는 날짜와 무관하게 모두 표시)

### 디버깅 원칙

**문제 발생 시:**
1. 프론트엔드 로그 확인 (Console)
2. 백엔드 로그 확인 (Vercel Logs)
3. **데이터베이스 직접 확인** (Supabase SQL Editor)
4. 근본 원인 찾기 전까지 임시방편 사용 금지

**v1.2에서 배운 교훈:**
- "유키노가 5개 추가했다고 했는데 2개만 보여" → DB에서 실제 데이터 확인 필수
- 날짜 필터링은 항상 **타임존**과 **과거 데이터** 고려
- GPT Function Calling 결과는 **반드시 검증 및 보정**
...
### 2025-11-29 - 태스크 필터링 로직 근본 개선
**핵심 문제 발견 및 해결:**

#### 🔴 CRITICAL BUG: 태스크가 표시되지 않는 문제
**증상:**
- 유키노가 "5개 태스크 추가했다"고 말함
- Supabase DB에는 8개 태스크 존재
- 프론트엔드에는 2개만 표시 (또는 0개)
...
SELECT * FROM tasks
WHERE user_id = '...'
  AND planned_at >= '2025-11-29T00:00:00.000Z'  -- 오늘
  AND planned_at < '2025-11-30T00:00:00.000Z';  -- 내일

-- 실제 DB 상태
planned_at: 2025-11-25 15:00:00+00  -- 4일 전에 생성된 태스크들
```

태스크들이 **11월 25일**에 생성되었는데, 필터는 **11월 29일(오늘) 태스크만** 조회하여 아무것도 표시 안 됨.

**해결:**
```typescript
// 이전: 날짜 기반 필터
.gte('planned_at', startOfDay)
.lt('planned_at', startOfTomorrow)
...
**변경 사항:**
- ✅ "오늘의 태스크" → "해야 할 태스크" (미완료 전체 표시)
- ✅ 날짜와 관계없이 **모든 미완료 태스크** 표시
- ✅ 과거 태스크도 완료 전까지 계속 표시
- ✅ 실용적이고 데이터 손실 방지

**추가 안정화 작업:**
- ✅ GPT가 잘못된 날짜 제공 시 자동 보정 로직 추가
  - 오늘이 아닌 날짜 → 강제로 오늘로 변경
  - 시간 부분 없으면 → 현재 시각 자동 추가
- ✅ 디버깅 로그 추가 (날짜 범위, 로드된 태스크 정보)
- ✅ System Prompt 강화: Function Calling 사용 명시
- ✅ Template literal 빌드 오류 수정 (backtick 제거)
...
- 🔴 **유키노 태스크 추가 후 홈화면 미반영**: 날짜 필터링 로직이 `.gte()/.lte()` 사용으로 시간대 문제 발생
  - **해결**: `.gte(startOfDay).lt(startOfTomorrow)` 패턴으로 변경 (v1.2에서 완전히 재설계)
